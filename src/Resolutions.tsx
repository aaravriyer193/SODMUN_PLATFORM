  ...
  return (
    ...
    >
      {[
        ...
        <button key={action} title={title} onClick={() => onAction(action)}
          style={{ background: 'transparent', border: '1px solid transparent', color: '#52525B', width: '30px', height: '30px', borderRadius: '7px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'all 0.1s' }}
...
const EditableBlock = ({ block, index, commitBlocks, blocks, handleKeyDown, getPrefix, isChair }: any) => {
  ...
  const applyFormat = (command: string) => {
    ...
    if (editorRef.current)
      commitBlocks(blocks.map((b: any) =>
        b.id === block.id ? { ...b, html: editorRef.current!.innerHTML, text: editorRef.current!.textContent || '' } : b
      ));
    setToolbarPos(null);
  };
  ...
  const handleLocalKeyDown = (e: React.KeyboardEvent<HTMLDivElement>) => {
    if (isChair && (e.ctrlKey || e.metaKey) && e.key.toLowerCase() === 'h') {
      e.preventDefault(); handleFormatAction('hilite');
    }
    handleKeyDown(e, index, block.id, editorRef.current);
  };

  // ── FIX: use px units and typed values ──────────────────────────────────
  ...
  const fontColor  = block.type === 'heading' ? '#F07C00' : '#27272A';
  ...
  return (
    <div style={{
      ...
      position: 'relative',
      cursor: 'text',
      // ── FIX: ensure block stays inside paper flow ──────────────────────
      width: '100%',
    ...
    >
      ...
      {block.type === 'point' && (
        <div style={{
          width: '36px', paddingTop: '3px', color: '#F07C00',
      ...
      <div
        ...
        style={{
          ...
          outline: 'none',
          // ── FIX: break-word so long strings stay in column ──────────────
          wordBreak: 'break-word',
          overflowWrap: 'break-word',
          whiteSpace: 'pre-wrap',
          marginBottom: block.type === 'heading' ? '16px' : '0px',
          letterSpacing: block.type === 'heading' ? '-0.5px' : '0px',
          textTransform: block.type === 'heading' ? 'uppercase' : 'none',
          // ── FIX: never let contenteditable overflow its parent ───────────
          maxWidth: '100%',
          boxSizing: 'border-box',
        }}
        spellCheck={false}
        data-placeholder={
          block.type === 'heading'   ? 'Resolution heading…' :
          block.type === 'point'     ? 'Clause…'             :
                                       'Write here…'
        }
        onInput={handleInput}
...
export default function Resolutions() {
      ...
      .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'resolutions', filter: `id=eq.${activeRes.id}` }, (payload) => {
        try {
          if (payload.new.title !== activeRes.title) {
            setActiveRes((p: any) => ({ ...p, title: payload.new.title }));
  ...
  const handleCreateResolution = async () => {
    ...
    const { data: res } = await supabase.from('resolutions')
      .insert([{ title: newResTitle, bloc_id: parseInt(selectedBlocId), committee: profile?.committee, content: initialContent }])
      .select().single();
  ...
  const openResolution = (res: any) => {
    ...
    try {
      ...
      setBlocks(Array.isArray(parsed) && parsed.length > 0
        ? parsed
        : [{ id: Date.now().toString(), type: 'heading', html: '', text: '', indent: 0 }]
      );
    } catch {
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLDivElement>, index: number, id: string, el: HTMLDivElement | null) => {
    ...
    if (e.key === 'Tab' && !e.shiftKey) {
      ...
      const newBlock = {
        ...
        type: block.type === 'point' ? 'point' : 'paragraph',
        html: '', text: '',
        indent: block.type === 'point' ? block.indent : 0,
      };
      const u = [...blocks];
      u.splice(index + 1, 0, newBlock);
      commitBlocks(u);
      setTimeout(() => {
        const nodes = document.querySelectorAll('[contenteditable]');
        if (nodes[index + 1]) (nodes[index + 1] as HTMLElement).focus();
      }, 10);
      ...
      if (block.type === 'point' && block.indent > 0)                   { const u = [...blocks]; u[index].indent -= 1; commitBlocks(u); }
        ...
        setTimeout(() => {
          const nodes = document.querySelectorAll('[contenteditable]');
          if (nodes[index - 1]) {
            const n = nodes[index - 1] as HTMLElement;
            n.focus();
  ...
  if (activeRes) {
    return (
      <div style={{ height: '100vh', display: 'flex', flexDirection: 'column', background: presentationMode ? '#fff' : '#F0EDE8', overflow: 'hidden' }}>
        <style>{`
          ...
          <div style={{ padding: '0 32px', height: '52px', background: 'rgba(255,255,255,0.92)', borderBottom: '1px solid rgba(0,0,0,0.08)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', backdropFilter: 'blur(8px)', flexShrink: 0, boxShadow: '0 1px 4px rgba(0,0,0,0.04)' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
              <button
                ...
                style={{ background: 'transparent', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '8px', color: '#71717A', fontSize: '13px', fontWeight: 600, fontFamily: 'Manrope, sans-serif', padding: '6px 10px', borderRadius: '8px', transition: 'all 0.12s' }}
            ...
            <div style={{ display: 'flex', gap: '8px' }}>
              {isChair && (
                <button className="primary-btn" style={{ fontSize: '11px', padding: '8px 16px' }} onClick={() => setPresentationMode(true)}>Present</button>
              )}
            </div>
          </div>
        )}
        ...
        {presentationMode && (
          <button
            ...
            style={{ position: 'fixed', top: '20px', right: '20px', padding: '8px 16px', background: 'rgba(0,0,0,0.75)', backdropFilter: 'blur(8px)', color: '#fff', borderRadius: '8px', zIndex: 100, border: 'none', fontWeight: 700, fontSize: '12px', cursor: 'pointer', fontFamily: 'Manrope, sans-serif' }}
          >
            Exit
          </button>
        )}

        {/* ── FIX: scroll container is the only thing that scrolls ──────── */}
        <div style={{
          flex: 1,
          overflowY: 'auto',
          overflowX: 'hidden',       // ← prevent horizontal escape
          padding: presentationMode ? '60px 40px' : '48px 60px',
        }}>
          {/* ── FIX: centre wrapper — no flexbox tricks that break flow ─── */}
          <div style={{
            margin: '0 auto',
            width: '100%',
            maxWidth: '816px',
          }}>
            {/* Paper — grows with content, never clips blocks ────────────── */}
            <div style={{
              background: '#fff',
              width: '100%',
              // ── FIX: minHeight gone — paper now auto-sizes to content ──
              ...
              boxSizing: 'border-box',
              // ── FIX: constrain all children horizontally ───────────────
              overflow: 'hidden',
            }}>
              {/* Title */}
              <input
              ...
              <div style={{ display: 'flex', justifyContent: 'center', marginBottom: '40px', gap: '8px', alignItems: 'center' }}>
                ...
                <span style={{ fontSize: '11px', fontWeight: 600, color: '#A1A1AA', textTransform: 'uppercase', letterSpacing: '1.5px', whiteSpace: 'nowrap' }}>
                  {profile?.committee} · {new Date().getFullYear()}
                </span>
                <div style={{ height: '1px', flex: 1, background: 'rgba(0,0,0,0.08)' }} />
              </div>
              ...
              {/* Bottom padding so last block isn't flush against paper edge */}
      ...
      <div className="panel" style={{ gridColumn: '1 / -1' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '14px' }}>
          {filteredResolutions.map(r => (
            <div
              key={r.id}
              onClick={() => openResolution(r)}
              style={{ background: 'rgba(255,255,255,0.85)', border: '1px solid rgba(0,0,0,0.08)', borderRadius: '14px', padding: '20px', cursor: 'pointer', transition: 'all 0.15s', display: 'flex', flexDirection: 'column', gap: '8px' }}
          ...
          {filteredResolutions.length === 0 && (
            <p style={{ color: '#A1A1AA', fontSize: '13px', gridColumn: '1/-1', padding: '20px 0' }}>No documents yet.</p>
          )}
        </div>
      </div>
